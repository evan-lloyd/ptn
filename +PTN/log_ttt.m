function [z, ind_compl_x, ind_compl_y] = log_ttt(x, y, ind_x, ind_y)
%log_TTT Tensor times tensor (full tensors, in log-space).
%
%  log_TTT(X,Y) computes the outer product of tensors X and Y
% 
%  log_TTT(X,Y,XDIMS,YDIMS) computes the contracted product of tensors X
...
% 
%  log_TTT(X,Y,DIMS) is equivalent to calling log_TTT(X,Y,DIMS,DIMS).
%
...
%  X = rand(4,2,3);
%  Y = rand(3,4,2);
%  Z = log_ttt(X,Y) %<-- outer product of X and Y
%  Z = log_ttt(X,X,1:3) %<-- inner product of X with itself
%  Z = log_ttt(X,Y,[1 2 3],[2 3 1]) %<-- inner product of X & permuted Y
%  Z = log_ttt(X,Y,[1 3],[2 1]) %<-- product of X & Y along specified dims
% 
%  See also TTM.
...
% ----------------------------------------------------------
% *** MODIFIED FOR PTN PACKAGE, OPERATIONS PERFORMED IN LOG-SPACE ***
...
% Perform the contraction in log-space with logmulexp
if(transX)
  if(~transY)
    Z = logmulexp(X', Y);
  else

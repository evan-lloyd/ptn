...
%                        annotation.
%             'semilog': Set if x-axis is log-scaled, to get properly sized
%                        boxes and whiskers.
% PARAMETER PAIRS
...
if horizontalFlag
    if logwidthFlag
        hReg2(end+1) = rectangle('Position',[boxEdge(1),y*10^(-barWidth/2),IQR,y*(10^(barWidth/2)-10^(-barWidth/2))],'linewidth',linewidth,'EdgeColor',boxColor,'facecolor',[1 1 1]);

        hold on;
        hReg2(end+1) = plot([medianX medianX],[y*10^(-barWidth/2) y*10^(barWidth/2)],'color',meanColor,'linewidth',linewidth);
        hReg2(end+1) = plot([boxEdge(1) boxEdge(2)],[y*10^(-barWidth/2) y*10^(-barWidth/2)],'linewidth',linewidth,'color',boxColor);
        ...
        hReg2(end+1) = plot([wisEdge(1) wisEdge(1)],[y*10^(-barWidth/3) y*10^(barWidth/3)],'-','linewidth',linewidth,'color',wisColor);
        hReg(end+1) = plot([wisEdge(2) wisEdge(2)],[y*10^(-barWidth/3) y*10^(barWidth/3)],'-','linewidth',linewidth,'color',wisColor);
    ...
    if logwidthFlag
        hReg2(end+1) = rectangle('Position',[y*10^(-barWidth/2),boxEdge(1),y*(10^(barWidth/2)-10^(-barWidth/2)),IQR],'linewidth',linewidth,'EdgeColor',boxColor,'facecolor',[1 1 1]);
        hold on;

        hReg2(end+1) = plot([y*10^(-barWidth/2) y*10^(barWidth/2)],[medianX medianX],'color',meanColor,'linewidth',linewidth);
        
        hReg2(end+1) = plot([y*10^(-barWidth/2) y*10^(-barWidth/2)],[boxEdge(1) boxEdge(2)],'linewidth',linewidth,'color',boxColor);
        ...
        hReg2(end+1) = plot([y*10^(-barWidth/3) y*10^(barWidth/3)],[wisEdge(1) wisEdge(1)],'-','linewidth',linewidth,'color',wisColor);
        hReg(end+1) = plot([y*10^(-barWidth/3) y*10^(barWidth/3)],[wisEdge(2) wisEdge(2)],'-','linewidth',linewidth,'color',wisColor);
...
if outlierFlag % but only if you want to
    ...
    if ~isempty(yy)
        ...
        if logwidthFlag
            % Displacement on linear scale would be this
            yy = (yy-y)*4+y;
            yy = (yy-y)*(barWidth/maxPointHeight)/max([yy-y; barWidth/maxPointHeight])+y;
            
            % Convert displacement to same visual separation on log-scale
            yy = y * 10.^(yy-y);
        else
            yy = (yy-y)*4+y;
            yy = (yy-y)*(barWidth/maxPointHeight)/max([yy-y; barWidth/maxPointHeight])+y;

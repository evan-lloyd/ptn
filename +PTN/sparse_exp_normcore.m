function [y, maxIdx, offset, expOffset]=sparse_exp_normcore(inp, tol, varargin)
% Function modified from TT-Toolbox for PTN package
% Specialization of amen_cross routine for taking the point-wise exponent
% of a tensor train.
% Normalizes each core such that the largest element is 1,
% to avoid passing extremely small values into the SVD.
% Additional (optional) return values include the index of the largest
% element found, the normalization factors for each core, and the
% additive offset passed in varargin (if used).
% --------
...
offset = -Inf * ones(X{1}.d);


% Avoid tensor of all zeros for stability
%expOffset = max(1 / prod(X{1}.n), realmin);
expOffset = 0;
...
d = numel(n);
if (isempty(y))
    % Initialize with first two terms of Maclaurin series for exp(x)
    y = 1 + X{1};
end;
ry = y.r;
y = core2cell(y);
...
%     end;
        ...
        % XXX
        % Restart sweep on offset change
        mf = max(fx(:));
        if mf > offset(i)
            offset(i) = mf;
            maxIdx = J(fx(:) == mf, :);
            if(dispLargeElements)
                fprintf('new offset (%d) at %s\n', offset, mat2str(J(fx(:) == mf, :)));
            end
            if dir == 1
                %i = 1;
                %continue;
        ...
        cry = exp(fx - offset(i)) + expOffset; % sizes: rnr x nx -> rnr x d2
    ...
    if (norm(cry, 'fro')==0)
        % Estimate got all small again, adjust the offset
        offset(i) = mf;
        cry = exp(fx - offset(i)) + expOffset;
...
%            end;
                ...
                crz = exp(fx - offset(i)) + expOffset; % sizes: rnr x nx -> rnr x d2
...
%            end;
                ...
                crs = exp(fx - offset(i)) + expOffset; % sizes: rnr x nx -> rnr x d2
...
%            end;
    ...
    if ((i==d+1)||(i==0))
        if (verb>0)
            fprintf('=amen_cross= swp=%d, max_dx=%3.3e, max_rank=%d, mean_rank=%d, #ffun_evals=%d\n', swp, max_dx, max(ry), round(mean(ry)), fevalcnt);
        end;
...
% Nudge a bit to avoid getting negative values
%if sum(y) < 0
%    y = y + expOffset / 2;
%end
...
if verb > 0
    sprintf('sparse_exp offset: %d', offset)
end

function [ net, parms ] = log_tn_eval(net,parms,order)
  % Evaluate the given tensor network with the given evaluation order.
  % Assumes parameters are in log-space, and returns a one-node network
  % containing the log of the evaluation.

  if numel(order) == 1
      return;
  end
  
  [net,parms] = PTN.log_tn_eval(net, parms, order{1});
  v1 = min(flat(order(1)));
  ...
  parms{min(v1,v2)} = PTN.log_ttt(parms{v1}, parms{v2}, t, v);
  net = merge(net, v1, v2);
  parms(max(v1,v2)) = [];
  ...
  [net,parms] = PTN.log_tn_eval(net, parms, order);
